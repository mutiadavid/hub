...
const DocumentSidebarComponent = ({
  ...
  const extractName = React.useCallback((value) => {
    if (!value) return null;
    if (typeof value === "string") return value;
    return value.name || value.fullName || value.userName || null;
  }, []);

  const extractRole = React.useCallback((value) => {
    if (!value) return null;
    if (typeof value === "string") return value;
    return value.role || value.userRole || null;
  }, []);

  const resolveUploaderInfo = React.useCallback((doc) => {
    const uploadedBy =
  ...
  }, [extractName, extractRole]);

  const resolveUploadDate = React.useCallback((doc) =>
    doc.uploadedAt ||
    doc.uploadDate ||
    doc.createdAt ||
    doc.updatedAt ||
  ...
  const allDocs = useMemo(() => {
    const processedDocs = (Array.isArray(documents) ? documents : [])
      .filter(
        (doc) =>
    ...
    const processedSupportingDocs = (Array.isArray(supportingDocs) ? supportingDocs : []).map(
      (doc, index) => {
        const uploaderInfo = resolveUploaderInfo(doc);
  ...
  }, [documents, supportingDocs, resolveUploaderInfo, resolveUploadDate]);

  const groupedDocs = useMemo(
    () =>
      allDocs.reduce((accumulator, doc) => {
