...
const DocumentSidebar = ({ documents = [], supportingDocs = [], open, onClose }) => {
  ...
  const allDocs = useMemo(() => {
      ...
      if (uploads.length > 0) {
        ...
        if (fileUrl) {
          ...
          processedDocs.push({
            ...
            title: doc.name || `Document ${index + 1}`,
            category: doc.category || "Main Documents",
            fileName: doc.fileName || fallbackFileName || doc.name || `Document ${index + 1}`,
            fileUrl,
    ...
    const processedSupportingDocs = (Array.isArray(supportingDocs) ? supportingDocs : []).map((doc, index) => {
      const uploaderInfo = resolveUploaderInfo(doc);
